...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "select mov_id, mov_title, role from movies, movies_actors where mov_title='Annie Hall' and mov_id='911';"
requests["Find the first and last names of all the actors who were cast in the movies 'Annie Hall', and the roles they played in that production"] = "select act_fname, act_lname, mov_title, role from movies, movies_actors, actors where mov_title='Annie Hall' and mov_id='911';"


requests["Find the name of movie and director who directed a movies that casted a role as Sean Maguire"] = "select distinct directors.id, dir_fname, dir_lname, actors.id, act_fname, act_lname, role, movies.id, mov_title
...
requests["Find all the actors who have not acted in any movie between 1990 and 2000 (select only actor first name, last name, movie title and release year)"] = "SELECT DISTINCT na.act_id, a.act_fname, a.act_lname, n.id,mov_title, n.mov_year
FROM movies n
